...
class SPMM(pl.LightningModule):
    def __init__(self, tokenizer=None, config=None, loader_len=0, no_train=False):
        ...
        with open('./SPMM/normalize.pkl', 'rb') as w:
            norm = pickle.load(w)
        self.property_mean, self.property_std = norm
        self.property_mean, self.property_std = self.property_mean, self.property_std
    ...
    @torch.no_grad()
    def get_smiles_emb(self, data):
        data_loader = ['[CLS]' + d for d in data['smiles']]
        text_input = self.tokenizer(data_loader, padding='longest', truncation=True,
                                    max_length=100, return_tensors="pt").to(self.device)
        text_input_ids, text_attention_mask = text_input.input_ids[:, 1:], text_input.attention_mask[:, 1:]
        ...
        return text_input, vl_embeddings
